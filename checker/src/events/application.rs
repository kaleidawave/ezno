...
use crate::{
	...
	types::{
		...
		subsititue, Constructor, StructureGenerics, TypeId, TypeStore,
	},
...
pub(crate) fn apply_event(
	...
	match event {
		...
		Event::SetsVariable(variable, value) => {
			let new_value = subsititue(value, type_arguments, environment, types);
		...
		Event::Getter { on, under, reflects_dependency } => {
			let on = subsititue(on, type_arguments, environment, types);
			let property = subsititue(under, type_arguments, environment, types);
		...
		Event::Setter { on, under, new, reflects_dependency, initialization } => {
			let on = subsititue(on, type_arguments, environment, types);
			let under = subsititue(under, type_arguments, environment, types);

			let new = match new {
				Property::Value(new) => {
					Property::Value(subsititue(new, type_arguments, environment, types))
				}
			...
			let with = with
				...
				.map(|argument| match argument {
					SynthesisedArgument::NonSpread { ty, position: pos } => {
						let ty = subsititue(*ty, type_arguments, environment, types);
		...
		Event::Throw(thrown) => {
			let subsititued_thrown = subsititue(thrown, type_arguments, environment, types);

			target.get_top_level_facts(environment).throw_value(subsititued_thrown);

			if subsititued_thrown != TypeId::ERROR_TYPE {
				return None;
			}
		}
		// TODO extract
		Event::Conditionally { condition, events_if_truthy, else_events } => {
			let condition = subsititue(condition, type_arguments, environment, types);
		...
		Event::Return { returned } => {
			let subsititued_returned = subsititue(returned, type_arguments, environment, types);

			if subsititued_returned != TypeId::ERROR_TYPE {
				return Some(subsititued_returned);
			} else {
		...
		Event::CreateObject { referenced_in_scope_as, prototype } => {
			...
			let new_object_id = match prototype {
				PrototypeArgument::Yeah(prototype) => {
					let prototype = subsititue(prototype, type_arguments, environment, types);
