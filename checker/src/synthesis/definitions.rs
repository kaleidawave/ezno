...
pub(super) fn type_definition_file<T: crate::FSResolver>(
	...
	root.new_lexical_environment_fold_into_parent(
		crate::Scope::Module { source },
		checking_data,
			...
			for statement in definition.declarations.iter() {
				match statement {
					...
					TypeDefinitionModuleDeclaration::TypeAlias(type_alias) => {
						...
						// 	(&interface.on, environment, checking_data),
						// );
			...
			for declaration in definition.declarations.into_iter() {
				match declaration {
					TypeDefinitionModuleDeclaration::Function(func) => {
						...
						let res = environment.register_variable_handle_error(
							func.name.as_str(),
							// TODO
					...
					TypeDefinitionModuleDeclaration::Variable(DeclareVariableDeclaration {
						...
						// TODO tidy up
						let variable_ty = synthesize_type_annotation(
							&type_restriction,
							environment,
							checking_data,
						);
