...
impl<'a> Lexer<'a> {
	pub(crate) fn new<'a>(script: &'a str, offset: Option<u32>, options: LexerOptions) -> Self {
	...
	pub(crate) fn next(&mut self) -> Result<Token<TSXToken, TokenStart>, (LexingErrors, Span)> {
		let mut characters = self.script.char_indices();
		...
		for (idx, chr) in characters {
			...
			macro_rules! push_token {
				($t:expr $(,)?) => {{
					return Ok(Token($t, TokenStart::new(start as u32 + offset)));
				}};
			}
			...
			match state {
						...
						GetNextResult::Result { result, ate_character } => {
							...
							match result {
								...
								TSXToken::MultiLineComment(_) => {
									state =
										LexingState::MultiLineComment { last_char_was_star: false };
									continue;
								}
								_ => {}
							}
				...
				LexingState::RegexLiteral {
					...
					if *after_last_slash {
						if !chr.is_alphabetic() {
							if start != idx {
								push_token!(TSXToken::RegexFlagLiteral(
				...
				LexingState::JSXLiteral {
					...
					match jsx_state {
						...
						JSXLexingState::TagName { ref mut direction, ref mut lexed_start } => {
							match chr {
								// Closing tag
